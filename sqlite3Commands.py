...
def create_table():
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS registros (dev_id int ,momento TEXT, id_card TEXT, autorizado INTEGER);')
...
def insert_into_registro(registro):
    print("->def insert_into_registro(registro):... id_card:"+registro.id_card+" Autorizado:"+str(registro.autorizado))
    c = conn.cursor()
